...
interface FilterComponentStateType {
  ...
  // projects?: Project[];
  filterProjectOptions?: ProjectLabel[];
}
...
type ProjectLabel = {
  ...
  value: string;
  type: string;
  children?: IterationLabel[];
}

class IterationFilterComponent extends React.Component<IterationFilterComponentPropsType, FilterComponentStateType> {
  constructor(props: IterationFilterComponentPropsType) {
    super(props);
    console.log("IterationFilterComponent", props);
    this.state = {
  ...
  convertToFilterOptions = (projects: Project[]) => {
    ...
    projects.forEach((p) => {
      const iterationChildren: IterationLabel[] = [];
      const roleChildren: RoleLabel[] = [];
      const option = {
        value: p.id,
        label: p.name,
        type: 'p',
      ...
      p.iterations?.forEach((i) => {
        const iterationChildrenItem: IterationLabel = {
          value: i.id,
    ...
    console.log('convertToFilterOptions', projectOptions);
    this.setState({
  ...
  onCascadeChange = (values: any[][]) => {
    ...
    values.forEach((v) => {
      const selectedProject = this.props.projects.find((p) => p.id === v[0]);
      if (!selectedProject) {
        return;
      }
      //填充project
  ...
  submitQuery = () => {
    ...
    // 参考: #onChange() 的逻辑
    this.props.changedAction.onChanged(this.state.selectedProjects || []);
  };
